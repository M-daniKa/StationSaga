...
public class UI_level1_station2 extends JFrame {
    ...
    // reuse the same track as Station 1 so the 4 cars carry over
    static trackLinkedList track = UI_level1_station1.track;
    ...
    public UI_level1_station2() {
        // load level 1, station 2 part of `level1.txt`
        this.levelData = levelDataLoader.loadLevel(1, 2);

        setTitle("Station Saga - Level 1: Add/Remove (Station 2)");
    ...
    private void updateControllerState(DialogueEntry entry) {
        // sticky: never revert permissions once enabled
    ...
    // -------------------- DIALOGUE GATING --------------------
    // fully open now so it never blocks, kept for future use if needed
    ...
    private ActionResult performDeleteByIndex(int index) {
        if (!canDelete)
            return ActionResult.notAllowed("You can only delete when instructed.");

        if (index < 0)
            return ActionResult.error("Index must be non-negative.");

        // 🚫 BLOCK REMOVING THE HEAD
        if (index == 0)
            return ActionResult.error("You cannot remove the engine (head) of the train.");
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new UI_level1_station2().setVisible(true));
    }
}
